...
public final class HostAndPort implements Serializable {
  ...
  public static HostAndPort fromParts(String host, int port) {
    if (!isValidPort(port)) {
      throw new IllegalArgumentException(String.format("Port out of range: %s", port));
    }
    HostAndPort parsedHost = fromString(host);
    if (parsedHost.hasPort()) {
      throw new IllegalArgumentException(String.format("Host has a port: %s", host));
    }
   ...
   *
   * <p>Note that this parser identifies IPv6 literals solely based on the presence of a colon.
   *
